...
function BookMockup() {
  return (
    <div style={{ width: "100%", height: "100%", minHeight: 320, background: "linear-gradient(135deg, #2a0808 0%, #3d0c0c 50%, #2a0808 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative" }}>
      {/* Book cover — matches kundli PDF cover */}
      <div style={{
        ...
        <div style={{ position: "relative", padding: "24px 18px 18px", display: "flex", flexDirection: "column", alignItems: "center", height: "100%", textAlign: "center", zIndex: 2 }}>
          ...
          {/* Center medallion — seated Ganesha */}
          <div style={{
      ...
      <span style={{ position: "absolute", bottom: 8, right: 10, fontSize: 9, color: "#d4a843", opacity: 0.5, fontStyle: "italic" }}>sample cover</span>
    </div>
  );
}
...
function CalendarMockup() {
  return (
    <div style={{ width: "100%", height: "100%", minHeight: 320, background: "linear-gradient(135deg, #2a0808 0%, #3d0c0c 50%, #2a0808 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative" }}>
      ...
      <span style={{ position: "absolute", bottom: 8, right: 10, fontSize: 9, color: "#d4a843", opacity: 0.5, fontStyle: "italic" }}>sample preview</span>
    </div>
  );
}
...
function ProductCard({ product, currentTier }: { product: Product; currentTier: Tier }) {
  const { t, lang } = useT();
    ...
    className="shop-product-card">
      ...
      <div style={{ padding: "22px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ marginTop: "auto", display: "flex", gap: 8, flexDirection: "column" }}>
          ...
          {currentTier !== "plus" && (
            <Link href="#" style={{ textAlign: "center", fontSize: 12, color: "#5c1a1a", textDecoration: "underline" }}>
              {currentTier === "free"
...
export default function ShopPreviewClient() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fffaf0" }}>
      {/* Tier simulator (preview only) */}
      <div style={{ position: "sticky", top: 0, zIndex: 10, background: "#fff", borderBottom: "1px solid #e5d5b5", padding: "10px 20px", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <strong style={{ fontSize: 12, color: "#5c1a1a", letterSpacing: "0.5px" }}>{t("पूर्वावलोकन — योजना निवडा", "PREVIEW — simulate tier", "पूर्वावलोकन — योजना चुनें")}:</strong>
        {(["free", "premium", "plus"] as Tier[]).map(tier => (
          <button key={tier} onClick={() => setCurrentTier(tier)}
            style={{
              padding: "5px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: "pointer",
              background: currentTier === tier ? "#3d0c0c" : "#fff",
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 20px 20px", textAlign: "center" }}>
        <h1 style={{ fontSize: 34, color: "#3d0c0c", margin: 0, fontWeight: 700, letterSpacing: "-0.5px" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "20px 20px 40px", display: "flex", flexDirection: "column", gap: 20 }}>
        {PRODUCTS.map(p => (
          <ProductCard key={p.id} product={p} currentTier={currentTier} />
        ))}
      </div>
      <style>{`
      ...
      <div style={{ background: "#3d0c0c", color: "#FFF8E7", padding: "28px 20px", marginTop: 20 }}>
        <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <Link href={`/${lang}/account`} style={{ padding: "12px 24px", background: "#d4a843", color: "#3d0c0c", borderRadius: 8, fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
      ...
      {/* Contextual placement notes (preview-only) */}
      <div style={{ maxWidth: 900, margin: "30px auto", padding: "20px 24px", background: "#fff", border: "1px dashed #d4a843", borderRadius: 10, fontSize: 13, color: "#5c1a1a" }}>
        <div style={{ fontWeight: 700, color: "#3d0c0c", fontSize: 14, marginBottom: 8 }}>
          📍 {t("इतर ठिकाणी दाखवण्याच्या जागा (प्रोडक्शनमध्ये):", "Other placement spots (in production):", "अन्य प्रदर्शन स्थान (प्रोडक्शन में):")}
        </div>
        <ul style={{ margin: 0, paddingLeft: 20, lineHeight: 1.7 }}>
          <li>{t("/calendar — डाउनलोड विभागाजवळ 'छापील दिनदर्शिका मागवा' बॅनर", "/calendar — 'Order printed calendar' banner near download section", "/calendar — डाउनलोड के पास 'मुद्रित पंचांग ऑर्डर' बैनर")}</li>
          <li>{t("/kundli/result — निकालानंतर 'छापील पुस्तक म्हणून मिळवा' कार्ड", "/kundli/result — 'Get as bound book' card after results", "/kundli/result — परिणाम के बाद 'बाउंड पुस्तक' कार्ड")}</li>
          <li>{t("/account payments टॅब — Plus अपग्रेड nudge", "/account payments tab — Plus upgrade nudge", "/account payments टैब — Plus अपग्रेड nudge")}</li>
          <li>{t("नेव्हबार — 'दुकान' लिंक", "Navbar — 'Shop' link", "नेवबार — 'शॉप' लिंक")}</li>
        </ul>
      </div>
    </div>
  );
