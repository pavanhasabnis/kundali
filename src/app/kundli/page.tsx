...
export default function KundliPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/astrology/kundli/result?${params.toString()}`);
    } catch {
  ...
  return (
    <div className="bg-[#FAFAF8] py-6"><div className="space-y-8 max-w-4xl mx-auto px-4 sm:px-6">
              ...
              {saved.map((s: { name: string; params: Record<string, string> }, i: number) => (
                <div key={i} className="flex items-center justify-between p-2 rounded-lg hover:bg-[#FFF8E7] transition">
                  <button onClick={() => {
                    ...
                    router.push(`/astrology/kundli/result?${q}`);
