  ...
  function PersonForm({ form, setForm, name, setName, onSubmit, which }: { form: typeof form1; setForm: (f: typeof form1) => void; name: string; setName: (n: string) => void; onSubmit: () => void; which: 1 | 2 }) {
    return (
      <div className="bg-white rounded-xl border border-stone-200 p-4 space-y-3">
        <input type="text" placeholder={t("नाव", "Name")} value={name} onChange={e => setName(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
        ...
        <div className="grid grid-cols-2 gap-2">
          <input type="number" placeholder={t("तास", "Hour")} value={form.hour} onChange={e => setForm({ ...form, hour: e.target.value })} className="px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          <input type="number" placeholder={t("मिनिटे", "Min")} value={form.minute} onChange={e => setForm({ ...form, minute: e.target.value })} className="px-3 py-2 border border-gray-300 rounded-lg text-sm" />
        </div>
        <button onClick={onSubmit} disabled={loading === which || !form.year} className="w-full py-2 rounded-lg text-sm font-bold text-white disabled:opacity-50" style={{ background: "linear-gradient(135deg, #5c1a1a, #3d0c0c)" }}>
          {loading === which ? t("गणना...", "Calculating...") : t("कुंडली बनवा", "Generate")}
        </button>
      </div>
    );
  }
    ...
    <div className="bg-[#FAFAF8] py-6">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 space-y-6">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-gray-900">{t("कुंडली तुलना", "Kundli Comparison")}</h1>
          <p className="text-gray-600 mt-2 text-sm">{t("दोन कुंडल्या शेजारी पहा — पती-पत्नी, पालक-मूल", "View two charts side-by-side — husband-wife, parent-child")}</p>
        </div>

        {/* Input Forms */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h3 className="text-sm font-bold mb-2" style={{ color: "#3d0c0c" }}>{t("पहिली कुंडली", "First Kundli")}</h3>
            <PersonForm form={form1} setForm={setForm1} name={name1} setName={setName1} which={1} onSubmit={() => fetchKundli(form1, setKundli1, 1)} />
          </div>
          <div>
            <h3 className="text-sm font-bold mb-2" style={{ color: "#3d0c0c" }}>{t("दुसरी कुंडली", "Second Kundli")}</h3>
        ...
        {kundli1 && kundli2 && (
          <div className="bg-white rounded-xl border border-stone-200 overflow-hidden shadow-sm">
            <div className="grid grid-cols-3 text-center text-sm font-bold py-3" style={{ background: "linear-gradient(135deg, #3d0c0c, #5c1a1a)", color: "#d4a843" }}>
              ...
              <div>{name1 || t("पहिली", "First")}</div>
              <div>{name2 || t("दुसरी", "Second")}</div>
            </div>

            {/* Lagna */}
            <div className="grid grid-cols-3 text-center text-sm py-2 border-b border-stone-100 bg-[#FFF8E7]">
              <div className="font-bold" style={{ color: "#5c1a1a" }}>{t("लग्न", "Lagna")}</div>
              <div>{t(kundli1.lagnaRashiMr, kundli1.lagnaRashi)}</div>
              <div>{t(kundli2.lagnaRashiMr, kundli2.lagnaRashi)}</div>
            </div>

            {/* Moon */}
            <div className="grid grid-cols-3 text-center text-sm py-2 border-b border-stone-100">
              <div className="font-bold" style={{ color: "#5c1a1a" }}>{t("चंद्र राशी", "Moon Sign")}</div>
              <div>{t(kundli1.moonRashiMr, kundli1.moonRashi)}</div>
              <div>{t(kundli2.moonRashiMr, kundli2.moonRashi)}</div>
            </div>

            {/* Nakshatra */}
            <div className="grid grid-cols-3 text-center text-sm py-2 border-b border-stone-100 bg-[#FFF8E7]">
              <div className="font-bold" style={{ color: "#5c1a1a" }}>{t("नक्षत्र", "Nakshatra")}</div>
              <div>{t(kundli1.moonNakshatraMr, kundli1.moonNakshatra)}</div>
              <div>{t(kundli2.moonNakshatraMr, kundli2.moonNakshatra)}</div>
            </div>
